...
void CheckDuplNClusters(TString tracfile = "data/523306/o2trac_its.root", TString output = ".", TString rootSave = "", int rofStart = 0, int rofEnd = -1, int windowSize = 2, bool includeSame = 1)
{
    ...
    { // Loop over timeframes
        ...
        if (includeSame)
        { // If includeSame == 1, then we also run comparisons between tracks in the same ROF.  Otherwise, skip
    ...
    TString prefix = "523306"; // insert any more information you'd like to see on the plot title here, like run numbers
    ...
    TString filename = Form("%s/plots_523306.pdf", output.Data());
    c1->Print(filename + "[");
    c1->Print(filename);
    c2->Print(filename);
    c3->Print(filename);
    ...
    double_t fraction = dupl / comps;
    std::cout << "Zeroes: " << zeroes << " Duplicates: " << dupl << " Total: " << comps << " Fraction: " << fraction << endl;
}