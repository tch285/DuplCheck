...
void CheckDuplRun(int runNum = 523306, int windowSize = 2, int rofStart = 0, int rofEnd = -1, bool includeSame = 1, bool save = 1) {
    fair::Logger::SetFileSeverity("error");
    gErrorIgnoreLevel = kWarning;

    // Obtain data from files:
    TString tracfile = TString::Format("./data/%i/o2trac_its.root", runNum);
    ...
    TString title1 = "F: " + std::to_string(totalFrames) + ", ROF: " + std::to_string(rofStart) + "/" + std::to_string(totalROFs) + "/" + std::to_string(windowSize) + "; #Delta#phi [rad]; #Delta#eta";
    h1->SetTitle(title1);
    ...
    if (save) {
        TFile *output = TFile::Open(Form("./saved_hists_%i_%s/%i.root", windowSize, lim.Data(), runNum), "RECREATE");
        h5->Write();
        h6->Write();
        ...
        output->WriteObject(&frac, Form("frac_%i", runNum)); // I store the vector in the TFile
        std::cout << "eff trk " << frac_track << " comp " << frac_comp << endl;;
        // LOGP(info, "Efficiencies: trk {} comp {}", frac_track, frac_comp);
        output->Close();
    }

    TString filename = TString::Format("plots/%i_%i_%i_%i_inclS_%i.pdf", runNum, rofStart, windowSize, totalROFs, includeSame);
    c1->Print(filename+"[");
    c1->Print(filename);
    c2->Print(filename);
    c3->Print(filename);
