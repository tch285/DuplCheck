...
void CheckDuplRunComb(int rofStart = 0, int rofEnd = -1, int windowSize = 2, bool includeSame = 1)
{
    ...
    int IR[] = {} int fraction[11];
    for (int run : runs)  {
        // Obtain data from files:
        TString tracfile = TString::Format("./data/%i/o2trac_its.root", run);
    ...
    TString filename = TString::Format("plots/all_ws_%i_inclS_%i.pdf", windowSize, includeSame);
    c1->Print(filename + "[");
    c1->Print(filename);
    c2->Print(filename);
    c3->Print(filename);
